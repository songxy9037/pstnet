...
class PSViTLayer(nn.Module):
    ...
    def forward(self,
        ...
        # print(sample_feat.shape)  # 4096,32
        # sample_feat = self._forward_semantic_tokens(x) # 8,32
...
class PSViT(nn.Module):
    ...
    def forward_feature(self, x):
        ...
        #
        # trans_out = self.norm(trans_out)    # 4,32
...
@register_model
def ps_vit(pretrained=False, num_iters=4, **kwargs):
    if pretrained:
    ...
    stem = nn.Sequential(nn.Conv2d(kwargs.get('in_chans', 3),
                         ...
                         BottleneckLayer(256, 64, 32))

    model = PSViT(embed_dim=32,
                  num_iters=num_iters,
                  depth=14,
                  num_heads=4,
                  mlp_ratio=3.,
